...
@router.post('/users/',response_model=schema.User, tags=["user"])
...
@router.put("/users/{user_id}", response_model=schema.User, tags=["user"])
def update_user_by_id(data: schema.User = None, user_id: int = None, db: Session = Depends(get_db)):
    db_user = crud.get_user_by_id(db, user_id=user_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return crud.update_user_active_by_id(data=data, db=db, user_id=user_id)
